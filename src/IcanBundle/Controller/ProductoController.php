...
class ProductoController extends BaseController
{
    ...
    public function indexAction() {
        ...
        return $this->render('IcanBundle:Producto:index.html.twig', array('categorias' => $categorias, 'marcas' => $marcas, 'ruta' => $ruta . $dir));
    }
    ...
    public function salvarAction(Request $request) {
        ...
        $resultadoJson = array();
        if ($producto_id == "") {
            $resultado = $this->SalvarProducto($categoria_id, $marca_id, $nombre, $titulo, $descripcion, $tags, $stock, $precio, $precioOferta, $mostrarPrecio, $estado, $fecha, $imagen, $imagenes, $productos);
        } else {
            $resultado = $this->ActualizarProducto($producto_id, $categoria_id, $marca_id, $nombre, $titulo, $descripcion, $tags, $stock, $precio, $precioOferta, $mostrarPrecio, $estado, $fecha, $imagen, $imagenes, $productos);
        }
    ...
    public function salvarImagenAction() {
        try {
            ...
            $imagine->open($dir . $archivo)->thumbnail($size, $mode)->save($dir . $archivo_thumb);
    ...
    public function EliminarRelacionado($productorelacion_id) {
        ...
        $entity = $this->getDoctrine()->getRepository('IcanBundle:ProductoRelacion')->find($productorelacion_id);
        ...
        if ($entity != null) {
            ...
            $relacion = $this->getDoctrine()->getRepository('IcanBundle:ProductoRelacion')->BuscarRelacion($producto_relacionado_id, $producto_id);
            if ($relacion != null) {
                $em->remove($relacion);
            }
    ...
    public function EliminarImagen($imagen) {
        ...
        $em = $this->getDoctrine()->getManager();
        $productoimagen = $this->getDoctrine()->getRepository('IcanBundle:ProductoImagen')->findOneBy(array('imagen' => $imagen));
        if ($productoimagen != null) {
            $em->remove($productoimagen);
        }

        $producto = $this->getDoctrine()->getRepository('IcanBundle:Producto')->findOneBy(array('imagen' => $imagen));
        if ($producto != null) {
            $producto->setImagen("");
        }
        $em->flush();
    ...
    public function CargarDatosProducto($producto_id) {
        ...
        if ($entity != null) {
            ...
            $relacionados = array();
            $productos = $this->getDoctrine()->getRepository('IcanBundle:ProductoRelacion')->ListarRelacionados($producto_id);
            $posicion = 0;
            foreach ($productos as $key => $producto_relacion) {
                ...
                if ($producto->getProductoId() != $producto_id) {
                    array_push($relacionados, array('productorelacion_id' => $producto_relacion->getProductorelacionId(), 'producto_id' => $producto->getProductoId(), 'nombre' => $producto->getNombre(), "categoria" => ($producto->getCategoria() != null) ? $producto->getCategoria()->getNombre() : "", "marca" => ($producto->getMarca() != null) ? $producto->getMarca()->getNombre() : "", "estado" => ($producto->getEstado()) ? 1 : 0, "imagen" => $ruta . $producto->getImagen(), "precio" => number_format($producto->getPrecio(), 0, ',', '.'), "fecha" => $producto->getFechapublicacion() != "" ? $producto->getFechapublicacion()->format("d/m/Y H:i") : "", "views" => $producto->getViews(), 'posicion' => $posicion));
                    $posicion++;
                }
            }
    ...
    public function EliminarProducto($producto_id) {
        ...
        if ($entity != null) {
            ...
            $relacionados = $this->getDoctrine()->getRepository('IcanBundle:ProductoRelacion')->ListarRelacionados($producto_id);
            foreach ($relacionados as $relacionado) {
                $em->remove($relacionado);
            }

            //Eliminar los productos relacionados
            $productorelacionados = $this->getDoctrine()->getRepository('IcanBundle:ProductoRelacion')->ListarProductosRelacionado($producto_id);
            foreach ($productorelacionados as $relacionado) {
                $em->remove($relacionado);
            }

            //Eliminar vistas
            $producto_views = $this->getDoctrine()->getRepository('IcanBundle:ProductoView')->ListarViewsDeProducto($producto_id);
     ...
     * @author Marcel
     */
    public function ActualizarProducto($producto_id, $categoria_id, $marca_id, $nombre, $titulo, $descripcion, $tags, $stock, $precio, $precioOferta, $mostrarPrecio, $estado, $fecha, $imagen, $imagenes, $productos) {
        $em = $this->getDoctrine()->getManager();
        ...
        if ($entity != null) {
            ...
            $i = 1;
            $paux = $this->getDoctrine()->getRepository('IcanBundle:Producto')->BuscarPorUrl($url);
            ...
            if ($imagenes != "") {
                ...
                foreach ($imagenes as $value) {
                    if ($value != "") {
                        ...
                        $productoimagen = $this->getDoctrine()->getRepository('IcanBundle:ProductoImagen')->findOneBy(array('imagen' => $value));
                        if ($productoimagen == null) {
            ...
            if (count($productos) > 0) {
                foreach ($productos as $value) {
                    ...
                    $productoRelacion = $this->getDoctrine()->getRepository('IcanBundle:Producto')->find($relacionado_id);
                    if ($productoRelacion != null) {

                        $productorelacion = $this->getDoctrine()->getRepository('IcanBundle:ProductoRelacion')->find($productorelacion_id);
                        if ($productorelacion == null) {
     ...
     * @author Marcel
     */
    public function SalvarProducto($categoria_id, $marca_id, $nombre, $titulo, $descripcion, $tags, $stock, $precio, $precioOferta, $mostrarPrecio, $estado, $fecha, $imagen, $imagenes, $productos) {
        $em = $this->getDoctrine()->getManager();
        $resultado = array();
    ...
    public function ListarProductosParaRelacionados($start, $limit, $sSearch, $iSortCol_0, $sSortDir_0, $categoria_id, $marca_id, $productos_id) {
        ...
        $lista = $this->getDoctrine()->getRepository('IcanBundle:Producto')->ListarProductosParaRelacionados($start, $limit, $sSearch, $iSortCol_0, $sSortDir_0, $categoria_id, $marca_id, $productos_id);
        ...
        foreach ($lista as $value) {
            ...
            $arreglo_resultado[$cont] = array("id" => $producto_id, "nombre" => $nombre, "categoria" => $categoria, "marca" => $marca, "estado" => $estado, "imagen" => $imagen, "precio" => $precio, "fechapublicacion" => $fecha, "views" => $views, "acciones" => $acciones);
    ...
    public function TotalProductosParaRelacionados($sSearch, $categoria_id, $marca_id, $productos_id) {
        $total = $this->getDoctrine()->getRepository('IcanBundle:Producto')->TotalProductosParaRelacionados($sSearch, $categoria_id, $marca_id, $productos_id);
    ...
    public function ListarProductos($start, $limit, $sSearch, $iSortCol_0, $sSortDir_0, $categoria_id, $marca_id) {
        ...
        $lista = $this->getDoctrine()->getRepository('IcanBundle:Producto')->ListarProductos($start, $limit, $sSearch, $iSortCol_0, $sSortDir_0, $categoria_id, $marca_id);
        ...
        foreach ($lista as $value) {
            ...
            $arreglo_resultado[$cont] = array("id" => $producto_id, "nombre" => $value->getNombre(), "categoria" => ($value->getCategoria() != null) ? $value->getCategoria()->getNombre() : "", "marca" => ($value->getMarca() != null) ? $value->getMarca()->getNombre() : "", "estado" => ($value->getEstado()) ? 1 : 0, "imagen" => $imagen, "precio" => number_format($value->getPrecio(), 0, ',', '.'), "fechapublicacion" => $value->getFechapublicacion() != "" ? $value->getFechapublicacion()->format("d/m/Y H:i") : "", "views" => $value->getViews(), "acciones" => $acciones);
    ...
    public function TotalProductos($sSearch, $categoria_id, $marca_id) {
        $total = $this->getDoctrine()->getRepository('IcanBundle:Producto')->TotalProductos($sSearch, $categoria_id, $marca_id);
    ...
    public function ListarCategoriasArbol($estado = "1") {
        $tree = array();
        //Categoria Padres
        $categoria_padres = $this->getDoctrine()->getRepository('IcanBundle:Categoria')->ListarPadres($estado);
        //Resto de categoria
        $categoria_hijos = $this->getDoctrine()->getRepository('IcanBundle:Categoria')->ListarHijos($estado);
        ...
        for ($i = 0; $i < count($categoria_padres); $i++) {
            ...
            $subcategoria = $this->getDoctrine()->getRepository('IcanBundle:Categoria')->ListarCategoriasDelPadre($categoria_id, $estado);
            $class = (count($subcategoria) > 0) ? "optionGroup" : "";

            $element = array('categoria_id' => $categoria_id, 'categoria_padre_id' => "", 'descripcion' => $value->getNombre(), 'class' => $class);
    ...
    public function getChilds($tree, $categoria_hijos, $master_id, $class_count = 0, $categoria_padre_id) {
        ...
        for ($i = 0; $i < count($categoria_hijos); $i++) {
            ...
            if ($value->getCategoriaPadre()->getCategoriaId() == $master_id) {
                ...
                $hijos = $this->getDoctrine()->getRepository('IcanBundle:Categoria')->ListarCategoriasDelPadre($value->getCategoriaId(), "");
                $class_padre_hijo = (count($hijos) > 0) ? "optionGroup" : "";

                $element = array('categoria_id' => $value->getCategoriaId(), 'categoria_padre_id' => $categoria_padre_id, 'descripcion' => $value->getNombre(), 'class' => "optionChild$class_count $class_padre_hijo");
