...
class PerfilController extends BaseController
{
    ...
    public function CargarDatosPerfil($perfil_id) {
        ...
        $entity = $this->getDoctrine()->getRepository('IcanBundle:Rol')->find($perfil_id);
        if ($entity != null) {
    ...
    public function EliminarPerfil($rol_id) {
        ...
        if ($rol != null) {
            $usuarios = $this->getDoctrine()->getRepository('IcanBundle:Usuario')->ListarUsuariosRol($rol_id);
            if (count($usuarios) > 0) {
    ...
    public function EliminarPerfiles($ids) {
        ...
        if ($ids != "") {
            ...
            foreach ($ids as $perfil_id) {
                if ($perfil_id != "") {
                    $perfil = $this->getDoctrine()->getRepository('IcanBundle:Rol')->find($perfil_id);
                    if ($perfil != null) {
                        $usuarios = $this->getDoctrine()->getRepository('IcanBundle:Usuario')->ListarUsuariosRol($perfil_id);
                        if (count($usuarios) == 0) {
    ...
    public function ActualizarPerfil($rol_id, $nombre) {
        ...
        $entity = $this->getDoctrine()->getRepository('IcanBundle:Rol')->find($rol_id);
        if ($entity != null) {
            //Verificar nombre
            $rol = $this->getDoctrine()->getRepository('IcanBundle:Rol')->BuscarPorNombre($nombre);
            if ($rol != null) {
    ...
    public function ListarPerfiles($start, $limit, $sSearch, $iSortCol_0, $sSortDir_0) {
        ...
        $lista = $this->getDoctrine()->getRepository('IcanBundle:Rol')->ListarRoles($start, $limit, $sSearch, $iSortCol_0, $sSortDir_0);
        ...
        foreach ($lista as $value) {
            ...
            $arreglo_resultado[$cont] = array("id" => $perfil_id, "nombre" => $value->getNombre(), "acciones" => $acciones);
    ...
    public function TotalPerfiles($sSearch) {
        $total = $this->getDoctrine()->getRepository('IcanBundle:Rol')->TotalRoles($sSearch);
